...
def main(args=None):
    ...
    # use our dataset and deloafined transformations
    csv = pd.read_csv(trx_dir+'Datasets/Opacidades/TX-RX-ds-20210308-03_ubuntu.csv')
    #csv = csv[csv.image_source.isin(['jsrt','mimic','hiba'])].reset_index()
    ...
    train_idx, test_idx = train_test_split(image_ids,stratify=image_sources,test_size=0.2,random_state=42)
    ...
    pretrained_checkpoint = experiment_dir+'/18-03-21/maskRCNN-8.pth'
    ...
    num_epochs = 10
    output_dir = experiment_dir+'/19-03-21/'
    loss_type_weights = {
                "loss_classifier": 3,
                "loss_box_reg": 1,
                "loss_image_level": 1,
    ...
    for epoch in range(num_epochs):
        ...
        saving_path = '{}/maskRCNN-{}.pth'.format(output_dir,epoch)
